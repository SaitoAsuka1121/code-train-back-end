...
def login():
    ...
    driver.find_element(By.NAME,'email').send_keys('1940977566@qq.com')
    driver.find_element(By.NAME,'password').send_keys('acm12345678')
    driver.find_element(By.CLASS_NAME,'cur-p').click()
    time.sleep(2)
    while True:
        img_bg = None
        img_saw = None
        while img_bg is None:
            img_bg = driver.find_element(By.CLASS_NAME,
                'yidun_bg-img').get_attribute('src')
            img_saw = driver.find_element(By.CLASS_NAME,
        ...
        slide = driver.find_element(By.CLASS_NAME,'yidun_slider')
...
def submit(code, problem_id, exam_id,compiler):
    problem = 'https://zoj.pintia.cn/problem-sets/91827364500/problems/%s' % problem_id
    driver.get(problem)
    time.sleep(0.5)
    #####
    data = {"details": [{"problemSetProblemId": "%s" % problem_id, "programmingSubmissionDetail": {"compiler": "%s" % compiler,
                                                                                                   "program": "%s" % code}}]}
...
if __name__ == "__main__":
    ...
    res = submit(code, problem_id, exam_id,compiler)
    print(res)
    driver.quit()
